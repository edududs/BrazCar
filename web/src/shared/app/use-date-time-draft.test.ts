...
describe("splitInstant and joinInstant", () => {
  it("round-trip an instant through the local day and clock", () => {
    const iso = new Date("2026-09-24T19:00:00").toISOString();
...
describe("useDateTimeDraft", () => {
  it("starts from the instant, changes the day and steps the clock by five minutes", () => {
    const { result } = renderHook(() =>
      useDateTimeDraft(new Date("2026-09-24T18:30:00").toISOString()),
    );
    ...
    expect(result.current.value()).toBe(new Date("2026-09-26T18:35:00").toISOString());
  });
});
