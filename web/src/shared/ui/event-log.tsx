...
const entryTone = {
  ...
  marker: "bg-brand-soft text-brand-ink font-semibold",
...
export function EventLog({ entries, formatClock, visible = 200 }: EventLogProps) {
  return (
    <ol
      ...
      className="flex-1 overflow-y-auto rounded-field border border-line font-mono text-xs"
    >
      {entries
        .slice(-visible)
        .reverse()
        .map((entry) => (
          <li key={entry.id} className={`flex gap-2 px-2 py-0.5 ${entryTone[entry.tone]}`}>
            <time className="shrink-0 text-ink-3">{formatClock(entry.atMs)}</time>
